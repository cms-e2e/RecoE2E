...
void QGTagger::runEvtSel_jet ( const edm::Event& iEvent, const edm::EventSetup& iSetup, e2e::Frame2D& vJetSeeds ) {
   ...
   edm::LogInfo("QGTagger") << " >> Reading and selecting Jets from " << jets->size() << " jet seeds: ";
   for (unsigned iJ=0;iJ<jets->size();iJ++){
	...
	// Jet selection criteria
		...
		//std::cout<<"     * Selection failed at Jet index: "<<iJ<<" because abs(pt) < minJetPt_ --> pt: "<<std::abs(iJet->pt())<<" minJetPt_: "<<minJetPt_<<". Adding "<<iphi_<<" to JetSeediphi and "<<ieta_<<" JetSeedieta vectors."<<std::endl;
    		edm::LogInfo("QGTagger") << "     * Selection failed at Jet index: " << iJ << " because abs(pt) < minJetPt_ -->pt: " << std::abs(iJet->pt()) << " minJetPt_: " << minJetPt_ << ". Adding " << iphi_ << " to JetSeediphi and " << ieta_ << " JetSeedieta vectors."; 
		nJet++;					 
	}
		...
		//std::cout<<"     * Selection failed at Jet index: "<<iJ<<" because abs(eta) > maxJetEta_ --> eta: "<<std::abs(iJet->eta())<<" maxJetEta_: "<<maxJetEta_<<". Adding "<<iphi_<<" to JetSeediphi and "<<ieta_<<" JetSeedieta vectors."<<std::endl;
    		edm::LogInfo("QGTagger") << "     * Selection failed at Jet index: " << iJ << " because abs(eta) > maxJetEta_ --> eta: " << std::abs(iJet->eta()) << " maxJetEta_: " << maxJetEta_ << ". Adding " << iphi_ << " to JetSeediphi and " << ieta_ << " JetSeedieta vectors."; 
		nJet++;					  
	}
	...
	edm::LogInfo("QGTagger") << "     * Failed Jet seed at index: " << iJ << " seed is too close to the edge of HE. Adding -1 to JetSeediphi and JetSeedieta vectors."; 
	ieta_=-1;
	iphi_=-1;
    	nJet++;
      	continue;
